// Package main is the entry point for the OpenPrint Analytics Service.
// This service provides analytics and reporting for print jobs, printers, and users.
package main
...
import (
	...
	"github.com/openprint/openprint/services/analytics-service/handler"
...
func main() {
	...
	aggregator := processor.NewAggregator(db)

	// Create handler
	h := handler.New(aggregator)
	...
	middlewareChain := middleware.Chain(
		middleware.LoggingMiddleware(log.New(os.Stdout, "[ANALYTICS] ", log.LstdFlags)),
		middleware.RecoveryMiddleware(log.New(os.Stdout, "[ANALYTICS] ", log.LstdFlags)),
		...
		middleware.CORSMiddleware([]string{"*"}, []string{"GET", "OPTIONS"}, []string{"Content-Type", "Authorization"}),
	)
