...
# ═══════════════════════════════════════════════
# PHASE RECOVERY (detect and fix stuck phases)
# ═══════════════════════════════════════════════
...
detect_stuck_phase() {
  ...
  status=$(state_get "$phase" status 2>/dev/null || echo "pending")
  [ "$status" != "running" ] && return 1  # Not stuck if not running

  updated=$(state_get "$phase" _updated 2>/dev/null || echo "0")
  [ -z "$updated" ] && return 1

  local now age
  now=$(date +%s)
  updated=$(date -d "$updated" +%s 2>/dev/null || echo "$now")
  age=$((now - updated))

  [ "$age" -gt "$timeout" ]
}

# Auto-recover from stuck phases
recover_stuck_phase() {
  local phase="$1"
  local timeout="${2:-600}"

  if detect_stuck_phase "$phase" "$timeout"; then
    warn "⚠️  STUCK DETECTED: $phase idle for >${timeout}s"
    slog "⚠️  STUCK DETECTED: ${phase} idle for >${timeout}s — resetting to pending"

    # Reset phase state
    state_set "$phase" status pending
    state_set "$phase" _updated "$(date -Iseconds)"

    # Record in error history
    record_error "$phase" "stuck_phase" "Phase was stuck for >${timeout}s, auto-recovered"
...
# Check all phases for stuck states and recover
check_and_recover_stuck_phases() {
  local timeout="${1:-600}"
  local phases=(requirements market_research design backend frontend testing qa security deploy)
  local recovered=0

  for phase in "${phases[@]}"; do
    if recover_stuck_phase "$phase" "$timeout"; then
      recovered=$((recovered + 1))
    fi
  done

  [ "$recovered" -gt 0 ] && slog "✅ Recovered $recovered stuck phase(s)"
  return "$recovered"
}
...
[TRUNCATED — original was ${prompt_len} chars. Focus on the most important parts above.]"
  ...
  _claude_retry_cmd() {
    ...
    if ! command -v claude >/dev/null 2>&1; then
      warn "  ⚠️  Claude CLI not found - skipping"
      exit_code=127
      return 1
    fi
...
[REDUCED — Claude was killed. Simplified prompt.]"
      ...
      # Exit code 1 might still have valid output - check log file
      if [ -s "$log_file" ] && grep -q "Done\|Complete\|Success" "$log_file" 2>/dev/null; then
        warn "  ⚠️  Exit code 1 but output looks valid - continuing"
        ok=true
        return 0
      fi
    fi
  ...
  # Don't fail hard - allow pipeline to continue
  warn "  ⚠️  $role_name failed but continuing (fault-tolerant mode)"
  return 0  # Changed from 1 to 0 for fault tolerance
}
...
# Health check: detect and recover from stale state
health_check() {
  local skip_check="${1:-false}"

  [ "$skip_check" = "true" ] && return 0

  # Check if PID file exists but process is dead (stale state)
  if [ -f "$PID_FILE" ]; then
    local pid; pid=$(cat "$PID_FILE" 2>/dev/null)
    if [ -n "$pid" ] && ! kill -0 "$pid" 2>/dev/null; then
      warn "⚠️  Stale PID file detected (process $pid dead) - cleaning up"
      slog "Cleaning stale PID file: $pid"
      rm -f "$PID_FILE"

      # Check for stuck phases and reset them
      check_and_recover_stuck_phases 300 >/dev/null 2>&1 || true
    fi
  fi
...
case "$CMD" in
  start)
    ...
    health_check "$SKIP_HEALTH_CHECK"
  ...
  phase)
    # Health check before running phase
    health_check "$SKIP_HEALTH_CHECK"
  ...
  status)
    # Health check before showing status
    health_check true
    show_status
    ;;
  recover)
    # Manual recovery command
    echo "Checking for stuck phases..."
    if check_and_recover_stuck_phases "${2:-600}"; then
      echo "✅ Recovered stuck phases"
    else
      echo "No stuck phases found"
    fi
    ;;
  ...
  "")             show_help ;;
  # Tolerate unknown commands (e.g., "fg", "loop" from stdin) - log but don't crash
  *)              warn "Unknown command ignored: $CMD"; slog "Ignoring unknown: $CMD"; exit 0 ;;
esac
